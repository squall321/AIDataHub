...
(division/team uppercased) and that consecutive calls without inserts
...
async def test_next_seq_normalizes_division_team_case(test_session):
    """``division="he"`` 와 ``"HE"`` 는 같은 카운터로 취급되어야 한다."""
...
async def test_next_seq_repeated_calls_no_inserts(test_session):
    ...
    a = await next_seq(test_session, data_type="DOC", division="HE", team="CAE", year=2026)
    b = await next_seq(test_session, data_type="DOC", division="HE", team="CAE", year=2026)
    assert a == b == 1
