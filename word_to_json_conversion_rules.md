...
```
{output_dir}/
├── HE-CAE-2026-000001.json
└── HE-CAE-2026-000001/                ← 폴더명 = doc_id
    ├── F001.png
    ├── F002.jpeg
    └── F003.emf
```

- 폴더명: `doc_id` 그대로 (예: `HE-CAE-2026-000001`).
...
2. 변환기가 PostgreSQL에 다음 쿼리:
   SELECT MAX(순번) FROM documents WHERE division='HE' AND team='CAE' AND year=2026

3. 다음 순번 = MAX + 1, 6자리 패딩
4. doc_id 조립: HE-CAE-2026-000034
```
...
```
output/HE-CAE-2026-000001.json
output/HE-CAE-2026-000001.warnings.log    ← 경고 모음
```
...
```python
def convert(docx_path, division, team, year):
    # 1. 사전 검증
    ...
    # 3. doc_id 발급
    doc_id = issue_new_doc_id(division, team, year)
...
output/
├── HE-CAE-2026-000001.json
├── HE-CAE-2026-000001.warnings.log
└── HE-CAE-2026-000001/
    ├── F001.png
    └── F002.png
...
*본 변환 규칙서는 [json_schema_rules.md](./json_schema_rules.md) 의 v1.0 출력 스키마를 기준으로 작성되었으며, 스키마가 변경될 때 함께 갱신됩니다.*
