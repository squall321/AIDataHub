...
| 차트 도형              | `tables[]` (categories+series → headers/rows, `ppt_converter/charts.py`) + `attachments[kind=chart]` placeholder + `blocks[type=table_ref]` |
...
```
figure id : DOC-{div}-{group}-{year}-{seq:06d}-F{nnn}
table id  : DOC-{div}-{group}-{year}-{seq:06d}-T{nnn}
attach id : DOC-{div}-{group}-{year}-{seq:06d}-A{nnn}
```
...
- 차트의 일부 종류 (3D / Surface 등) — 지원되는 종류는 11.5 절 참조
...
### 11.5 차트 — series·categories 자동 추출 (`ppt_converter/charts.py`)

차트 도형 (`shape.has_chart=True`) 은 `chart.plots[*].series[*]` 를 순회해 `Table` 형태로 추출한다. 결과는 슬라이드의 `tables[]` 에 1행으로 추가되며, 본문 트리에는 `block.type=table_ref` 로 참조된다. 동시에 차트 placeholder 가 `attachments[].kind="chart"` 로 등록되어 원본 위치/제목이 보존된다.

| 차트 종류 | 매핑 | 비고 |
|---|---|---|
| Bar / Column / Line | `headers=["category", "<series1.name>", ...]`, `rows=[[cat, val1, ...], ...]` | 표준 케이스 |
| Pie / Doughnut | `headers=["category", "<series.name>"]`, `rows=[[cat, val], ...]` | 단일 series |
| Scatter (XY) | categories 자리에 X 값 | series 별 (X, Y) 페어 |
| 3D / Surface / Bubble | 미지원 → placeholder 만 + 경고 | 향후 검토 |

**캡션**: 차트의 `chart_title` (있으면) → `tables[].caption`. 없으면 `"Chart on slide N"` 자동 생성.

**비활성화**: `--no-extract-charts` (charts.py 호출 자체를 skip → 11.0 이전 동작과 동일).
...
*본 변환 규칙서는 [json_schema_rules.md](./json_schema_rules.md) 의 v1.3 출력 스키마 (호환성: v1.0 JSON 페이로드 그대로 사용 가능)와 `src/ppt_converter/` (python-pptx 기반) 구현체를 기준으로 작성되었으며, 스키마/구현이 변경될 때 함께 갱신됩니다.*
