...
## AI 친화적 문서 데이터 표준 v1.1 (DB-precision · pgvector-aware)

> 작성일: 2026-05-07 (개정: 2026-05-10 — v1.1)
> 적용 대상: **Word · Excel · PPT · MD · PDF · HTML** 6개 변환기에서 산출된 모든 사업부 문서 JSON
> 소비자: `api_server/src/api/ingest/normalizer.py` → PostgreSQL `records` / `record_sections` / `record_attachments`
> 시작점: 변환기·룰 문서·구현 매핑은 [`CONVERSION_RULES_INDEX.md`](./CONVERSION_RULES_INDEX.md) 참조.

### v1.1 변경 요약 (2026-05-10)

- 적용 대상에 **PDF · HTML** 추가 (각각 `pdf_to_json_conversion_rules.md`, `html_to_json_conversion_rules.md`)
- **embedding 필드** (벡터 검색용 `embedding(384)` / `embedded_at` / `embedding_model`) 명세 — 13장 DB 매핑 표 갱신
- **agent_hints** / **related_record_ids** / **query_examples** / **access_pattern** 메타 필드 정식화 (4장)
- **attachments** 가 모든 변환기 (Word/PPT/MD/PDF/HTML) 의 정식 최상위 키임을 본문에서 확정 (이전: PPT 전용 표기)
- **사용 가능 임베더**: `hash` (기본 폴백) / `openai` (text-embedding-3-small, 1536-dim) / `e5_small` (multilingual-e5-small, 384-dim, 한국어 권장)
...
본 규칙서는 변환기(Word/Excel/PPT/MD/PDF/HTML)와 데이터베이스(PostgreSQL + pgvector) 사이의 **단일 계약(contract)** 이다.

- 모든 변환기는 본 스키마를 따르는 JSON을 생성한다.
- `api_server/src/api/ingest/normalizer.py`는 본 스키마에 정의된 경로만 신뢰한다.
...
문서의 신원증명서. AI가 식별·분류·인용·필터링하는 데 사용한다.

> **변환기 출력 vs DB 컬럼 (중요)** — 변환기가 출력하는 `meta` 키 이름과 DB `records` 테이블의 컬럼 이름은 일부가 의도적으로 다르다 (역사적 이유). normalizer (`api_server/src/api/ingest/normalizer.py`) 가 alias 폴백으로 흡수하므로 두 표기 모두 통과한다. 본 문서는 **실제 변환기 출력 형식 (= 코드 진실)** 을 기준으로 표기하고, DB 컬럼은 §13 매핑 표에서 별도 정리한다.
...
### 4.1 식별 필드 — `doc_id` 단일 PK

| 필드 | 타입 | 필수 | 규칙 |
|------|------|------|------|
| `doc_id` | string | **필수** | 12장 형식 (`{prefix}-{team}-{group}-{year}-{seq6}`). 전사 고유. **변환기가 실제 출력하는 키**. normalizer 는 `meta.doc_id` 우선, 없으면 `meta.id` / `raw.id` 폴백. DB `records.id` 컬럼으로 매핑. |
| `id` | string | (alias) | 위 폴백 경로용. 사용 안 권장 — 신규 변환기는 `doc_id` 만 출력. |

> **참고**: `data_type` / `team` / `group` / `year` / `seq` 는 `doc_id` 에서 파싱으로 도출되며 **변환기는 별도 키로 출력하지 않는다**. RecordIn/RecordOut (Pydantic) 의 `data_type` / `team` / `group` / `year` / `seq` 필드는 normalizer 의 `parse_id()` (`api_server/src/api/schemas/id_format.py`) 가 채운다.

> **예외**: Excel DATA payload 만 top-level 에 `data_id` / `team` / `group` / `year` 를 별도 출력한다 (§11.2 참조).

### 4.2 RAG 필드

| 변환기 출력 키 | DB 컬럼 (RecordIn) | 타입 | 필수 | 규칙 |
|---|---|------|------|------|
| `title` | `title` | string | 필수 | 원본 제목. 버전 표기 금지 (별도 `version`). |
| `summary` | `summary` | string | 권장 | 1~3문장, 최대 500자. RAG 1차 필터. (Word/PPT 자동 추출 미지원 — override 만, 7대 작성 표준 필요) |
| `tags` | `tags` | string[] | 권장 | 2~20개. 짧은 키워드. 부서·작성자·날짜 금지. (Word/PPT 자동 추출 미지원) |
| **`agent_scope`** | `agents` | string[] | 권장 | 참조해야 하는 에이전트 식별자. **변환기는 `agent_scope`** 로 출력. normalizer 가 `meta.agent_scope` 우선, 없으면 `raw.agents` 폴백 (`normalizer.py:108`). DB·API 응답은 `agents`. |
| `domain` | `domain` | string | 선택 | 도메인 분류 (구조해석/제어/품질 등). **현재 변환기 자동 추출 없음** — Excel `_META.domain` 만 수동 기재. |
| `subject_keywords` | `subject_keywords` | string[] | 선택 | tags 보조. 한국어 정규 키워드. **현재 Excel `_META.subject_keywords` 만 지원**. |
...
### 4.4 분류/생애주기 필드 (Migration 0006)

| 필드 | 타입 | 기본값 | 허용값 |
|------|------|------|------|
| `classification` | enum | `"internal"` | `public` / `internal` / `confidential` / `restricted` |
| `status` | enum | `"draft"` | `draft` / `review` / `approved` / `deprecated` |
| `language` | string | `"ko"` | ISO 639-1 (`ko`, `en`, ...) |
| `parent_record_id` | string | `null` | 다른 record 의 `id`. FK. |
| `derivation` | enum | `"original"` | `original` / `extracted` / `aggregated` / `translated` |
| `quality_score` | integer | `null` | 0~100. SMALLINT. |
| `valid_from` | date | `null` | `"YYYY-MM-DD"`. |
| `valid_until` | date | `null` | `"YYYY-MM-DD"`. |
...
**classification / status / derivation / access_pattern:** 위 §4.4 / §4.4-bis 표 참조.

**`derivation` 변경 이력**: 2026-05-09 까지 명세는 `original / revision / translation / extract` 로 표기되었으나, 코드 (`api_server/src/api/schemas/common.py:26`) 의 실제 enum 은 `original / extracted / aggregated / translated` 이다. 본 문서 v1.1 부터 코드를 단일 진실 공급원으로 채택.

### 4.6 변환기 own-extras (포맷별 부가 정보)

각 변환기는 본 명세의 표준 키 외에 자기 포맷 특유의 메타를 다음 위치에 출력한다. normalizer 는 이들을 검증하지 않고 `records.content` JSONB 에 그대로 보존한다 (변경 시 ingest 영향 없음).

| 변환기 | 위치 | 키 | 내용 | 코드 |
|---|---|---|---|---|
| **Word** | `meta.agent_scope` (top) | — | 위 §4.2 참조. 별도 own-extras 컨테이너는 없음 (모두 표준 키) | `converter/core.py:809-810` |
| **Excel** | top-level | `data_id` / `schema_version="data.v1"` / `team` / `group` / `year` / `caption` / `headers` / `units` / `units_map` / `rows` / `row_count` / `column_count` / `source.{sheet,kind}` / `generated_at` / `warnings` | DATA payload 자체가 표준 변종. §11.2 참조 | `excel_converter/core.py:125-159` |
| **Excel** | `tables[].context` | `description` / `method` / `condition` / `equipment` / `operator` / `date` / `notes` / `caveats` | `_META: sheet:<name>.*` | `excel_converter/core.py:354-363` |
| **Excel** | `tables[].column_descriptions` | `{컬럼명: 의미}` | `_GLOSSARY` description | `excel_converter/core.py:495` |
| **PPT** | `meta.agent_scope` (top) | — | 동상. PPT 슬라이드 본문은 표준 sections 트리 | `ppt_converter/core.py:907` |
| **MD** | `meta.front_matter_extra` | YAML front matter 의 미매핑 키 | 표준 키 (`title`/`tags`/`agents`/`summary`/`author`/`doc_type`/`created`/`modified`/`version`) 외 모든 frontmatter | `md_converter/core.py:649-656` |
| **HTML** | `meta.head_meta_extra` | `<meta name=...>` 미매핑 | 표준 매핑 안 된 head meta + `_extra` | `html_converter/core.py:641-650` |
| **PDF** | `meta.pdf` | `{page_count, heading_strategy, creator, producer, creation_date, modification_date, ocr_pages?}` | PDF 작성 SW 식별 + 휴리스틱 출처 + OCR 적용 페이지 | `pdf_converter/core.py:651-664` |

작은 모델이 own-extras 를 인식할 때는 `meta.{front_matter_extra,head_meta_extra,pdf}` 만 추가로 살피면 된다.
...
"sections": [
  {
    ...
    "figure_refs": ["DOC-HE-CAE-2026-0000000001-A001"],
    "table_refs":  ["DOC-HE-CAE-2026-0000000001-T001"],
    "children":    []
  }
]
```
...
"attachments": [
  {
    ...
    "section_ref": "1.2",
    "source_ref":  "DOC-HE-CAE-2026-0000000001-S001",
...
### 9.3 kind 11종 — `api/schemas/attachment.py` 권위 출처

| 값 | 의미 | 대표 확장자 / mime_type |
|----|------|----------------|
| `figure` | 비트맵·벡터 이미지 (Word/PPT 인라인, 차트 캡쳐 포함) | png/jpg/jpeg/gif/bmp/wmf/emf/svg/tif/tiff/webp |
| `document` | 문서 첨부 (PDF, DOCX, HWP 등) | pdf/doc/docx/hwp/hwpx/txt/rtf/odt |
| `spreadsheet` | 스프레드시트 첨부 | xlsx/xls/xlsm/csv/tsv/ods |
| `media` | 오디오·비디오 | mp3/wav/ogg/flac/mp4/avi/mov/mkv/webm/m4a |
| `archive` | 압축 묶음 | zip/tar/gz/7z |
| `cad` | CAD 형상 (MCAD 원본·형상 교환·ECAD 원본) | step/stp/iges/igs/x_t/x_b/catpart/stl (ODB++/ECAD-JSON 은 인제스트 시 직접 지정) |
| `cae` | 솔버 입출력 덱/결과 | k/key/dyn/dynain/inp/cdb/odb/rad/bdf/nas/fem/op2/d3plot (확장자 없는 d3plot·dynain 등은 파일명 접두로 추론) |
| `drawing` | 도면 / 벡터 드로잉 | vsd/vsdx/svg (드로잉 컨텍스트) |
| `data` | 정형 데이터 (JSON/XML/YAML 등) | json/xml/yaml/yml |
| `chart` | PPT 차트 placeholder (변환기가 직접 지정, 확장자 추론 없음) | — |
| `other` | 위에 속하지 않는 모든 첨부 | (자유) |

**확장자 ↔ kind 자동 매핑**: `api/schemas/attachment.py:54+` `_KIND_BY_EXT` 표 참조. 알 수 없는 확장자는 `other`.

**PPT 차트**: `ppt_converter/charts.py` 가 추출한 차트 데이터는 `tables[]` 1행으로 들어가고, 추가로 `attachments[].kind="chart"` placeholder 가 등록된다 (`api/schemas/attachment.py` 의 chart kind).
...
"sources": [
  {
    "id":          "DOC-HE-CAE-2026-0000000001-S001",
    "type":        "MCAD",
    "format":      "CATPart",
...
### 11.1 DOC (Word·PPT·MD·PDF·HTML)

- 정상적인 본문 트리. `sections[].blocks`에 paragraph/code/list_item 등이 모두 등장.
- `tables`, `attachments` 모두 사용.
- **PDF**: 텍스트 추출 우선. 스캔 PDF 의 빈 페이지는 OCR opt-in (`--ocr`, [`pdf_to_json_conversion_rules.md`](./pdf_to_json_conversion_rules.md) §13 참조).
- **HTML**: head meta (title/description/keywords/og:*) → `meta` 매핑, h1~h6 → 섹션, table/img → tables/attachments. ([`html_to_json_conversion_rules.md`](./html_to_json_conversion_rules.md) 참조).
- **PPT**: 차트 도형은 `ppt_converter/charts.py` 가 series·category 를 추출해 `tables[]` 1행으로 추가 (Excel 의 표와 같은 구조). 차트 placeholder 는 `attachments[].kind="chart"` 로도 함께 등록.

### 11.2 DATA (Excel) — payload 별도 변종 `data.v1`

> **중요**: Excel 변환기는 본 명세 §3 의 표준 7-키 구조 (`schema_version="1.0"` + `meta` + `sections` + ...) 를 **그대로 따르지 않는다**. 시트 단위 데이터 표를 1차 시민으로 두는 별도 페이로드 형식 `schema_version="data.v1"` 를 출력한다. normalizer 의 `_extract_data()` (`api_server/src/api/ingest/normalizer.py:147-160`) 가 이 변종을 감지해 표준 RecordIn 으로 흡수한다.

**Excel 출력 top-level 키 (코드 진실)** — `excel_converter/core.py:125-159` `to_payload()`:

```jsonc
{
  "data_id":         "DATA-HE-CAE-2026-0000000034",
  "schema_version":  "data.v1",
  "caption":         "SS400 인장시험 결과",
  "team":        "HE",
  "group":            "CAE",
  "year":            2026,
  "headers":         ["strain", "stress"],
  "units":           {"strain": "%", "stress": "MPa"},      // 컬럼 → 단위 객체
  "units_map":       {"strain": "%", "stress": "MPa"},      // 동일 내용 alias (호환용)
  ...
  "generated_at":    "2026-05-08T12:34:56+09:00",
  "warnings":        [],
  "meta": { ...워크북 단위 메타 (옵션)... }
}
```

normalizer 흡수 결과 (`_extract_data`):

- `data_id` / `caption` → `RecordIn.id` / `RecordIn.title`
- `headers` / `rows` / `units` / `notes` → `content` JSONB
- `meta.*` → 표준 RecordIn 필드 (DOC 변종과 동일 폴백 규칙)

**`units` vs `units_map`**: 코드는 두 키를 모두 출력한다 (`excel_converter/core.py:145, 158`). 의미와 값이 동일. 신규 소비자는 `units` 사용 권장 (명세 단일화 방향).
...
| JSON path | 의미 | 출처 |
|---|---|---|
| `meta.title`, `meta.summary`, `meta.tags`, `meta.agent_scope`, `meta.domain`, `meta.classification`, `meta.status`, `meta.language`, `meta.source_system`, `meta.author`, `meta.department`, `meta.project`, `meta.version`, `meta.subject_keywords` | 워크북 단위 메타 — 모든 시트가 공유 | `_META` 워크북 키 / 빌트인 속성 |
...
| `tables[].units` / `tables[].units_map` | `{column_name: "단위"}` (객체 형식 — 컬럼 → 단위 매핑). 코드는 두 키를 모두 출력 (호환용 alias) | `_GLOSSARY` 의 unit / 헤더 인라인 |
...
| `GROUP` | 그룹 대문자 (CAE/MFG/QA/DEV/PLM ...) |
| `YYYY` | 4자리 연도 |
...
| Attachment | `{record_id}-A{3자리}` | `DOC-HE-CAE-2026-0000000001-A001` |
| Table | `{record_id}-T{3자리}` | `DOC-HE-CAE-2026-0000000001-T001` |
| Source | `{record_id}-S{3자리}` | `DOC-HE-CAE-2026-0000000001-S001` |
| Figure (legacy) | `{record_id}-F{3자리}` | `DOC-HE-CAE-2026-0000000001-F001` |
...
- `meta.id`의 `DIV`는 `meta.team`과 일치.
- `meta.id`의 `GROUP`은 `meta.group`과 일치.
...
> **JSON path 해석 규칙** (코드 진실): normalizer 의 `_extract_doc()` / `_extract_data()` / `_common_fields()` 가 폴백 체인을 따라 입력 위치에서 RecordIn 으로 흡수한다. 표의 "JSON path" 는 **변환기가 실제 출력하는 위치**(1차 경로) 를 적었으며, 폴백은 별도 칼럼에 표기.

| JSON path (1차) | 폴백 (alias) | type | required | DB column | DB type | notes |
|---|---|---|---|---|---|---|
| `meta.doc_id` (DOC), `data_id` (top, Excel DATA) | `raw.id`, `meta.id` | string | required | `records.id` | VARCHAR(80) PK | 12장 형식. 파싱하여 team/group/year/seq 도출 (`schemas/id_format.parse_id`) |
| (raw.data_type 또는 id prefix 파싱) | — | enum | required | `records.data_type` | VARCHAR(10) | DOC/DATA/SIM/CAD/LOG/FORM/OTHER. `detect_variant` 가 도출 |
| (id 파싱) | — | string | derived | `records.team` | VARCHAR(10) | RecordOut 응답에만 노출 |
| (id 파싱) | — | string | derived | `records.group` | VARCHAR(10) | RecordOut 응답에만 노출 |
| (id 파싱) | — | integer | derived | `records.year` | SMALLINT | RecordOut 응답에만 노출 |
| (id 파싱) | — | integer | derived | `records.seq` | INTEGER | RecordOut 응답에만 노출 |
| `meta.title` | `raw.title` | string | required | `records.title` | TEXT | Word RAG meta A-3 자동 추출 가능 |
| `meta.summary` | `raw.summary` | string | recommended | `records.summary` | TEXT | Word A-3 자동 추출 (lead-3, ≤500자) |
| `meta.tags[]` | `raw.tags` | array<string> | recommended | `records.tags` | TEXT[] | GIN. Word A-3 RAKE 자동 보강 |
| **`meta.agent_scope[]`** | `raw.agents`, `meta.agents` | array<string> | optional | `records.agents` | TEXT[] | GIN. 변환기 출력 키는 `agent_scope` |
| `schema_version` | — | string | required | `records.schema_version` | VARCHAR(10) | "1.0" (Excel 의 `data.v1` 은 normalizer 가 흡수 시 "1.0" 으로 정규화) |
| (전체 JSON) | — | object | required | `records.content` | JSONB | 본 JSON 원본 보관 |
| (computed) | — | string | computed | `records.content_hash` | VARCHAR(64) | 14장 산출 |
| `meta.source_file` | `raw.source_file` | string | optional | `records.source_file` | TEXT | 파일명만 |
| `meta.author` | `raw.author` | string | optional | `records.author` | VARCHAR(200) | |
| `meta.department` | `raw.department` | string | optional | `records.department` | VARCHAR(100) | |
| `meta.project` | `raw.project` | string | optional | `records.project` | VARCHAR(200) | |
| `meta.version` | `raw.version` | string | optional | `records.version` | VARCHAR(40) | 원본 표기 |
| `meta.classification` | `raw.classification` | enum | default `internal` | `records.classification` | VARCHAR(20) | public/internal/confidential/restricted |
| `meta.status` | `raw.status` | enum | default `draft` | `records.status` | VARCHAR(20) | draft/review/approved/deprecated |
| `meta.domain` | `raw.domain` | string | optional | `records.domain` | VARCHAR(100) | |
| `meta.subject_keywords[]` | `raw.subject_keywords` | array<string> | optional | `records.subject_keywords` | TEXT[] | |
| `meta.source_system` | `raw.source_system` | string | optional | `records.source_system` | VARCHAR(50) | |
| `meta.language` | `raw.language` | string | default `ko` | `records.language` | VARCHAR(10) | ISO 639-1 |
| `meta.parent_record_id` | `raw.parent_record_id` | string | optional | `records.parent_record_id` | VARCHAR(80) FK | records.id 참조 |
| **`meta.derivation`** | `raw.derivation` | enum | default `original` | `records.derivation` | VARCHAR(20) | **`original` / `extracted` / `aggregated` / `translated`** (`schemas/common.py:26` 권위) |
| (computed) | — | string[] | computed | `records.capabilities` | TEXT[] | normalizer 자동 산출 (현재 미연결 — A-5 로드맵) |
| `meta.quality_score` | `raw.quality_score` | integer | optional | `records.quality_score` | SMALLINT | 0~100 |
| `meta.valid_from` | `raw.valid_from` | date | optional | `records.valid_from` | DATE | YYYY-MM-DD |
| `meta.valid_until` | `raw.valid_until` | date | optional | `records.valid_until` | DATE | YYYY-MM-DD |
| **`meta.agent_hints`** | `raw.agent_hints` | string | optional | `records.agent_hints` | TEXT | 변환기 자동 생성 (A-2) |
| `meta.related_record_ids[]` | `raw.related_record_ids` | array<string> | optional | `records.related_record_ids` | TEXT[] | 수동 큐레이션 |
| `meta.query_examples[]` | `raw.query_examples` | array<string> | optional | `records.query_examples` | TEXT[] | 변환기 자동 생성 (A-2, title/tags 기반) |
| `meta.access_pattern` | `raw.access_pattern` | enum | default `occasional` | `records.access_pattern` | VARCHAR(20) | frequent/occasional/rare |
...
| `meta.doc_id` (또는 `data_id` / `meta.id` 폴백) | string | required | `record_sections.record_id` | VARCHAR(80) FK | records.id |
...
| (computed by job) | vector | optional | `record_sections.embedding` | VECTOR(384) | **활성** — `e5_small` 사용시 384-dim. `EMBEDDING_PROVIDER` 환경변수로 선택 (hash 폴백 / openai / e5_small). 인덱스: `ivfflat (embedding vector_cosine_ops) WITH (lists=100)` |
| (computed by job) | timestamptz | optional | `record_sections.embedded_at` | TIMESTAMPTZ | 임베딩 산출 시각 |
| (computed by job) | string | optional | `record_sections.embedding_model` | VARCHAR(100) | 사용된 모델명 (예: `intfloat/multilingual-e5-small`) |

**평탄화 규칙:** `sections` 트리는 깊이 우선 순회로 행렬화. `children`은 별도 컬럼 없이 `level`+`section_id` 조합으로 트리 복원.

**임베딩 산출:** ingest 시 `AUTO_EMBED_ON_INSERT=true` 면 자동, 그 외에는 `POST /api/jobs/embed` 또는 백필 스크립트로 일괄 처리. semantic 검색 (`/api/search?mode=semantic`) 은 `embedding <=> qvec` (코사인 거리) 로 top-K. 권장 임계값: hash 0.85 / openai 0.80 / e5 0.92.
...
| `attachments[].kind` | enum | required | `record_attachments.kind` | VARCHAR(20) | 9.3절 11종 |
...
- [ ] `meta.doc_id` (DOC 변종) 또는 top-level `data_id` (Excel DATA) 가 `{DATA_TYPE}-{DIV}-{GROUP}-{YYYY}-{6digits}` 형식인가
- [ ] 위 식별자에서 파싱한 `data_type` 이 enum 값 (DOC/DATA/SIM/CAD/LOG/FORM/OTHER) 중 하나인가
- [ ] `classification` / `status` / `derivation` / `access_pattern` 이 enum 값 중 하나인가 (기본값 적용 후) — `derivation` 허용값: `original` / `extracted` / `aggregated` / `translated`
- [ ] `tags` 가 2개 이상, 20개 이하인가
- [ ] `summary` 가 비어있지 않고 제목 단순 반복이 아닌가 (Word/MD/HTML 은 자동 추출 폴백 가능)
- [ ] `language` 가 ISO 639-1 코드인가
- [ ] `parent_record_id` 가 명시된 경우 해당 record 가 존재하는가
- [ ] `quality_score` 가 0~100 범위인가
- [ ] `valid_from <= valid_until` 인가 (둘 다 명시된 경우)
- [ ] `meta.agent_scope[]` 의 각 값이 `agents` 테이블에 등록되어 있는가 (현재 strict 검증 미연결, A-9 로드맵)
...
본 표는 변환기 구현 시 참조용이다. 6개 변환기는 동일 스키마를 출력하되 아래 항목에서만 차이가 있다.

| 항목 | Word | Excel | PPT | MD | PDF | HTML |
|------|------|-------|-----|-----|-----|------|
| `data_type` | DOC | DATA | DOC (slide) | DOC | DOC | DOC |
| `sections` 깊이 | 1~3 (Heading 1/2/3) | 1 (시트별) | 1~2 (섹션/슬라이드별) | 1~6 (h1~h6) | 1~3 (outline / 패턴 / 폰트 크기) | 1~6 (h1~h6) |
| `blocks` types | paragraph / code / table_ref / figure_ref / list_item / quote | (없음, table 단일) | paragraph / figure_ref | paragraph / code / list_item / table_ref / figure_ref / quote | paragraph / table_ref / figure_ref | paragraph / code / list_item / table_ref / figure_ref / quote |
| `tables` | Word 표 그대로 | 시트 1개 = table 1개 (`detect_multi_tables` opt-in 시 시트당 N개) | 슬라이드 내 표 + **차트 추출 표** (`charts.py`) | MD `\|...\|` 표 | `pdfplumber.extract_tables()` (그리드 표) | `<table>` element |
| `attachments` | 인라인 image + OLE + chart | (없음) | 인라인 image + 차트 이미지 + 오디오/비디오 | external_link (외부 그림) | 페이지 image placeholder + OCR 결과 텍스트 | `<img>` external_link |
| OCR 지원 | (해당 없음) | (해당 없음) | (해당 없음) | (해당 없음) | **opt-in `--ocr`** (Tesseract + pdf2image) | (해당 없음) |
| 캡션 매칭 규칙 | `"Figure N:"` / `"그림 N:"` 텍스트 패턴 매칭 | 시트 이름 = 캡션 | 슬라이드 노트 + alt-text 결합 | `![alt](path)`의 `alt` | (자동 감지 불가 — placeholder + 검수) | `<figcaption>` |
| `meta.language` 기본 | `ko` | `ko` | `ko` | 파일 frontmatter 우선, 없으면 `ko` | `ko` | `<html lang>` 우선, 없으면 `ko` |
| 파싱 라이브러리 | `python-docx` | `openpyxl` | `python-pptx` | `markdown` + `markdown-it-py` | `pdfplumber` + `pypdf` (+ optional `pytesseract` + `pdf2image`) | `beautifulsoup4` + `lxml` |
| heading 4+ 처리 | `blocks[].type=heading_inline`으로 강등 | (해당 없음) | (해당 없음) | 트리에 그대로 포함 (level 4~6) | level 3 으로 collapse | 트리에 그대로 포함 (level 4~6) |
| 코드 블록 출처 | `Code` 스타일 단락 | (해당 없음) | 텍스트 박스에 monospace 폰트 | ` ``` ` fence | (감지 안됨 — 일반 paragraph) | `<pre><code>` |
| 헤딩 추론 우선순위 | Heading 스타일 (정확) | (시트 이름 = section title) | 섹션/슬라이드 타이틀 | `#` `##` 명시 | outline > 패턴 > 폰트 크기 (휴리스틱) | `<h1>~<h6>` |

**변환기 공통 의무:**

- `meta.doc_id` (DOC) / top-level `data_id` (Excel DATA) 는 변환기가 직접 생성하지 않는다 (사전 발급 시스템에서 받아옴).
- `content_hash`는 변환기가 채우지 않는다 (normalizer가 §14 에 따라 산출).
...
- `agents` 필드는 변환기가 `meta.agent_scope` 로 출력하며, normalizer 가 `RecordIn.agents` 로 매핑한다.
- `derivation` 허용값: `original` / `extracted` / `aggregated` / `translated`.
...
```json
{
  ...
  "meta": {
    "doc_id":           "DOC-HE-CAE-2026-0000000001",
    "title":            "IGA (Isogeometric Analysis) 가이드",
    "summary":          "KooRemapper v1.3.0의 IGA(등기하해석) 기능 사용 가이드. NURBS 기반 Trimmed Volume 방식으로 FE solid mesh를 IGA로 자동 변환하는 절차, YAML 설정 문법, 생성 파일 구조를 설명한다.",
    ...
    "valid_until":      null,
    "agent_hints":      "이 record 는 DOC 입니다. 주요 토픽: IGA, LS-DYNA, NURBS. 본문은 총 5 섹션, 1 표, 1 그림으로 구성됩니다.",
    "query_examples":   ["IGA 가이드 어떻게 사용해?", "IGA에 대해 알려줘", "IGA 가이드 관련 자료 보여줘"],
    "access_pattern":   "occasional"
  },
  // 참고: data_type/team/group/year/seq 는 doc_id 파싱으로 도출되며 변환기는 출력하지 않음.
  // 응답 (RecordOut) 에서는 team="HE", group="CAE", year=2026, seq=1 로 노출됨.
      ...
      "children": [
        {
          ...
          "blocks": [
            ...
            { "type": "table_ref", "id": "DOC-HE-CAE-2026-0000000001-T001" }
          ],
          "figure_refs": [],
          "table_refs":  ["DOC-HE-CAE-2026-0000000001-T001"],
          "children":    []
        },
        {
          ...
          "blocks": [
            ...
            { "type": "figure_ref", "id": "DOC-HE-CAE-2026-0000000001-A001" }
          ],
          "figure_refs": ["DOC-HE-CAE-2026-0000000001-A001"],
          "table_refs":  [],
          "children":    []
