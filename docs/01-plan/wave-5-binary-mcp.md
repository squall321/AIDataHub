...
## 5.9 P1.9 — 멀티 런타임 확장 (완료, v0.5.0)

P1 ~ P1.8 까지 Python 만 컨테이너화 가능 (`generate_def` 가 `python:slim` 만 분기). v0.5.0 에서 4 런타임 지원.

### 지원 런타임

| runtime | base image | %post | %runscript |
|---|---|---|---|
| `python` (3.10/3.11/3.12) | `python:{ver}-slim` | `pip install -r requirements.txt` (있으면) | `exec python /opt/tool/{script} "$@"` |
| `node` (18/20/22) | `node:{ver}-slim` | `npm ci --omit=dev` 또는 `npm install --omit=dev` | `exec node /opt/tool/{script} "$@"` |
| `jvm` (11/17/21) | `eclipse-temurin:{ver}-jre` | `lib/*.jar` 있으면 classpath 자동 구성 | `java -jar /opt/tool/{script}` (또는 classpath + main-class) |
| `dotnet` (net8.0/net9.0) | `mcr.microsoft.com/dotnet/runtime:{tag}` | self-contained 가정 (별도 install 없음) | `dotnet /opt/tool/{script}` (App.dll 등) |
| `binary` | (예약) | P2 | P2 |
| `wine` | (예약) | P2 — LGPL OK 이나 winetricks pre-cache 인프라 필요 | P2 |
...
args:
  - { name: csv_path, type: string, required: true }
  - { name: out_json, type: string, default: "/work/summary.json" }
args_style: long_flags
return:
  format: json
  capture_files: { enabled: true }
persist_output:
  enabled: true
  data_type: DATA
  team: HE
...
- `python_version` / `node_version` / `jdk_version` / `target_framework` — runtime 별 버전 키 (없으면 default — Python 3.12 / Node 20 / JDK 17 / net8.0)
- 그 외 키는 모든 런타임 동일 (`args`, `args_style`, `return`, `persist_output`, `platform_capability`, `resource_limits`, ...)

### 폐쇄망 사전 sif 절차 (필수)

외부 docker.io 가 막힌 환경에서 build 가능하게 하려면, **base sif 를 사전에 1 회 fetch + scp** 후 캐시 디렉토리에 배치.

```bash
# 1. 외부 접근 가능한 머신에서:
apptainer pull python-3.12-slim.sif        docker://python:3.12-slim
apptainer pull node-20-slim.sif            docker://node:20-slim
apptainer pull eclipse-temurin-17-jre.sif  docker://eclipse-temurin:17-jre
apptainer pull mcr.microsoft.com-dotnet-runtime-8.0.sif \
                                            docker://mcr.microsoft.com/dotnet/runtime:8.0

# 2. scp 또는 USB 로 target 서버에 전달

# 3. target 서버 캐시 디렉토리에 배치:
#    우선순위 (apptainer_build_svc.py):
#      ① $AIDH_BASE_SIF_DIR/<basename>.sif       (운영자 명시 — 권장)
#      ② <repo>/api_server/cache/bases/<basename>.sif
#      ③ <repo>/deploy/apptainer/cache/bases/<basename>.sif

mkdir -p /opt/aidh/base-sifs
mv *.sif /opt/aidh/base-sifs/

# 4. .env 에 명시:
echo "AIDH_BASE_SIF_DIR=/opt/aidh/base-sifs" >> deploy/apptainer/.env

# 5. restart.sh --api  → 다음 업로드부터 Bootstrap: localimage 사용
```

`<basename>` 명명 규칙: `<image>:<tag>` 의 `:` 와 `/` 모두 `-` 로 치환.

- `python:3.12-slim` → `python-3.12-slim.sif`
- `mcr.microsoft.com/dotnet/runtime:8.0` → `mcr.microsoft.com-dotnet-runtime-8.0.sif`

캐시 hit 시 def 파일은 `Bootstrap: localimage` + `From: <절대경로>` 로 자동 생성 (docker.io 접근 0회).

### 테스트/디버깅 토글

```bash
# 강제 docker bootstrap (캐시 무시) — CI / 단위 테스트:
AIDH_DISABLE_LOCALIMAGE=1 .venv/bin/python -m pytest tests/test_mcp_upload.py
```
...
| 항목 | 결과 |
|---|---|
| `test_generate_def_multi_runtimes` | 4 런타임 각각 base/runscript 합성 검증 |
| `test_generate_def_reserved_runtime_raises` | binary/wine 거절 (`NotImplementedError`) |
| Node 예제 (`examples/wave-5/node_csv_summary/`) | 외부 dep 0, csv → 컬럼별 통계 JSON |
| 단위 테스트 합계 | 58 PASS / 6 skip |

### 다음 단계 (P2 reserve)

- **binary** runtime: 정적 ELF 도 컨테이너 강제 (`scratch` 또는 `alpine:latest`) — 폐쇄망 sif 절차만 추가하면 즉시 가능
- **wine** runtime: `dockurr/wine` 또는 `tianon/wine` 베이스 + winetricks pre-cache 인프라 필요
- **GPU sif** (별 런타임은 아님 — CUDA 베이스 sif 옵션): `nvidia/cuda:12.x-runtime-ubuntu22.04` 사전 fetch
