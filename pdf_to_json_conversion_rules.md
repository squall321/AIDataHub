...
| 스캔 PDF (이미지로만 구성) | **변환 불가** (현재 OCR 미지원) |
...
- 스캔 PDF (이미지만 있는 PDF) → OCR 통합 후 향후 지원
...
이미지 바이너리는 **현재 추출하지 않는다** — 향후 OCR 통합 시 함께 처리.
...
### 11.1 스캔 PDF (이미지로만 구성)

- `pdfplumber.Page.chars` 가 비어 있음 → 변환기가 `warnings` 에 경고.
- `meta.pdf.heading_strategy = "none"`.
- **현재는 빈 본문이 등록된다** — OCR 처리 후 재변환 필요.
- 향후: Tesseract / paddleocr 통합 시 자동 OCR 옵션 추가 예정.
...
| 카테고리 | 상태 | 해결 계획 |
|---------|------|-----------|
| 스캔 PDF (이미지 only) | 미지원 (텍스트 0건 경고) | OCR(tesseract/paddleocr) 통합 후 지원 |
