# AGENT_API_GUIDE — AI Data Hub REST + MCP API (MEDIUM)
...
이 API 가 다루는 데이터의 도메인은 **사업부(division)·팀(team) 단위로 누적되는
산업 엔지니어링 산출물**이다. HE/EV/PT/DA/MX/VD 6 개 사업부의 CAE 해석 보고서,
...
1차 분류되고, 각 레코드는 사람이 읽을 수 있는 ID(`DOC-HE-CAE-2026-000001`)
...
|---|---|---|---|
| `id` | str(80) PK | no | `DOC-HE-CAE-2026-000001` 형식 (§4) |
| `data_type` | str(20) | no | enum 7 종: DOC/DATA/SIM/CAD/LOG/FORM/OTHER |
| `division` | str(10) | no | HE/EV/PT/DA/MX/VD (§4.4) |
| `team` | str(20) | no | CAE/Test/Design/BMS/AI/MFG/QA/PLM/... |
| `year` | int | no | 2020..2099 |
| `seq` | int | no | (data_type, division, team, year) 단위 시퀀스 |
...
Unique constraint: `(data_type, division, team, year, seq)`. 같은 튜플로 두 번
...
전체 예: `DOC-HE-CAE-2026-000001`. 정규식
...
```
{record_id}-A{number:03d}      예: DOC-HE-CAE-2026-000001-A001
{record_id}-F{number:03d}      (레거시 — figure 전용)
```

### 4.4 사업부 / 팀 시드 (`api/seed/divisions.py`)
...
`/api/meta/options` 가 권위적으로 위 시드를 노출한다 (`allow_custom.team=true`,
`allow_custom.division=false`).
...
| GET | `/api/meta/options` | UI 셀렉트박스 옵션 (5 분 캐시) | — | `version, divisions, teams, agents, classifications, statuses, derivations, languages, data_types, supported_extensions, max_upload_mb, allow_custom` |
...
| POST | `/api/convert/` | 파일 → JSON (DB 적재 없음) | multipart `file, division, team, year, seq, tags, agents, classification, domain` | converter 결과 dict |
...
```json
{
  "anchor_record": {"id":"DOC-HE-AI-2026-100001","title":"...","data_type":"DOC","tags":["AI","DigitalTwin"]},
...
### 8.5 분야별 통계 (data_type / division / agent 분포)
...
```
*예외*: 같은 (data_type, division, team, year, seq) 가 이미 있으면 409
...
| `division`, `team` | str | 분류 키 | `.division, .team` | `r["division"]` |
...
```
?data_type=DOC&division=HE&team=CAE&year=2026
