# AGENT_API_GUIDE_LARGE — Mobile eXperience AI Data Hub 깊이 있는 API 레퍼런스
...
**Mobile eXperience AI Data Hub** 는 사업부 (Home Entertainment / EV / Powertrain / Data Analytics / Manufacturing / Vehicle Development 등) 의 문서·표·시뮬레이션·CAD·로그·양식을 단일 PostgreSQL `records` 테이블에 정규화 저장하고, REST + MCP 두 채널로 노출하는 RAG 친화 데이터 허브다. 현재 PoC 단계 (`contract_version=1.0`).
...
|---|---|---|
| `GET /api/discover` | 카탈로그 (총 record 수, data_type/division/classification 분포, 등록된 agent 목록 + 각 agent 의 record 수 + sample_query, starting_points URL 모음) | 첫 호출. 60초 in-memory 캐시 — `?no_cache=true` 로 신선도 강제. |
...
### 2.1 사업부 / 팀 시드 (`api/seed/divisions.py`)

| DIVISION (사업부) | 의미 | 주요 TEAM |
|---|---|---|
...
이 시드는 실제 사업부와의 1:1 매핑이 아닌 **샘플 카탈로그**다 (운영 진입 시 확장). `/api/meta/options` 가 현재 등록된 division/team 목록을 반환한다 — hardcoded 하지 말고 이 응답을 받아라.
...
| HE / EV / PT / DA / MX / VD | (위 §2.1 참조) | 사업부 코드 |
...
2. POST /api/auth/keys
   ...
   body:   {"name": "iga-team", "agent_scopes": ["iga-analyst"], "department": "EV/CAE", "expires_at": "2026-12-31T00:00:00Z"}
   resp:   {"id": 7, "name": "iga-team", "key": "<plaintext_returned_once>", ...}
...
| 컬럼 | 타입 | NULL | 의미 | 인덱스 |
|---|---|---|---|---|
| `id` | str(80) PK | no | 사람이 읽는 코드 (`DOC-HE-CAE-2026-0000000001`) | PK |
| `data_type` | str(20) | no | enum (§13) | `idx_records_type` |
| `division` | str(10) | no | 사업부 코드 | `idx_records_div_team` |
| `team` | str(20) | no | 팀 코드 | (위 복합) |
| `year` | smallint | no | 4자리 연도 | `idx_records_year` |
| `seq` | int | no | (data_type, division, team, year) 단위 시퀀스 | (uniq) |
| `title` | text | no | 제목 | — |
...
| `department` | str(100) | no | 기본 "" (보통 "{div}-{team}") | — |
...
Unique: `(data_type, division, team, year, seq)` (자연 키 — 마이그레이션 0001 + 0006 진화). PK 는 `id` 문자열.
...
| 컬럼 | 타입 | NULL | 의미 |
|---|---|---|---|
| `id` | str(80) PK | no | `{record_id}-A{nnn}` (예: `DOC-HE-CAE-2026-0000000001-A001`) |
...
- "한 사업부의 모든 approved 문서 + sections": `/api/records?division=HE&status=approved` → 각 record 의 `/sections`.
...
| DIVISION | 2-4 uppercase ASCII | 사업부 식별, 팀명 충돌 방지 |
| TEAM | 2-5 uppercase ASCII | 팀 식별 |
| YEAR | 4 digits 2020-2099 | 연도별 슬라이스 즉시 가능 |
| SEQ | 6 digits zero-padded (000001-999999) | 자연 정렬 + 100만 record/year/team 가능 |

### 5.2 "왜 사람이 읽는 ID 인가" — UUID 가 아닌 이유

1. **로그/디버그 가독성** — `DOC-HE-CAE-2026-0000000001` 은 봐도 알지만 UUID 는 모른다.
2. **분류 키가 ID 안에 내장** — ID 만 보고 division/team/year 즉시 추출. ETL/리포팅 단순화.
...
| 식별자 | 형태 | 의미 |
|---|---|---|
| `doc_id` | `{DIVISION}-{TEAM}-{YEAR}-{SEQ}` (data_type prefix 없는 구버전) | DOC content 내부 self-reference. legacy. normalizer 가 수신 시 `DOC-` prefix 보강. |
...
### 5.4 Legacy ID — `{DIVISION}-{TEAM}-{YEAR}-{SEQ}` (data_type 누락)
...
| 메서드 | 경로 | 용도 |
|---|---|---|
| GET | `/api/records` | 목록 + 필터 (`data_type, division, team, year, agent[], tag[], q, include_deleted, limit, offset`) |
...
| 메서드 | 경로 | 응답 |
|---|---|---|
| GET | `/api/analytics/distribution` | counts (by_data_type/division/team/year) |
...
| 메서드 | 경로 | 응답 |
|---|---|---|
| GET | `/api/meta/options` | UI 셀렉트박스 (versions, divisions, teams, agents, classifications, statuses, derivations, languages, data_types, supported_extensions, max_upload_mb, allow_custom) |
...
| 메서드 | 경로 | 폼 |
|---|---|---|
| POST | `/api/convert/` | `file, division, team, year, seq, tags, agents, classification, domain` — 변환만 |
...
`seq=0` (또는 음수) → backend 가 `(data_type, division, team, year)` 단위 `MAX(seq)+1` 자동 할당. data_type 은 확장자에서 추정 (DOCX/PPTX/MD/PDF→DOC, XLSX→DATA).
...
4차: query 단순화 (문장 → 키워드)
5차: GET /api/records?division=X&data_type=Y (광역 슬라이스)
...
3. 신뢰도 합산 후 정렬
4. 응답에 신뢰도 + 인용 + 출처 명시
   "본 답변은 quality_score 87, approved, 작성자 홍길동 (HE/CAE) 의 DOC-HE-CAE-2026-0000000001 §S003 에 근거"
```
...
```
1. GET /api/analytics/distribution → by_data_type/division/team/year
...
1. multipart POST /api/convert/ingest
   file=<doc.docx>, division=HE, team=CAE, year=2026, seq=0,
...
2. interpreted_query.filters 검토 — 너무 협소한 필터?
   YES → 필터 일부 제거 (year/division 우선 제거)
...
| `division/team/year/seq` | str/int | 출처 분류 |
...
```json
{"version":"1.0","title":"Mobile eXperience AI Data Hub","total_records":1234,
...
```json
{"from":"DOC-HE-CAE-2026-0000000001","to":"DOC-HE-CAE-2026-0000000002",
...
**수동:**
```
POST /api/jobs/embed {"record_id": "DOC-HE-CAE-2026-0000000001"}
...
대량 적재용 명령행 도구:
```
python -m api.ingest.batch --root <dir> --recurse --division HE --team CAE --year 2026
```
...
| `idx_records_div_team` | division, team | 복합 filter |
...
2. **인용 단순화** — `[DOC-HE-CAE-2026-0000000001 §3.1]` 한 형식으로 record + section 동시 표현.
...
LIST      →  GET  /api/records?data_type=DOC&division=HE&year=2026&limit=20
...
INGEST    →  multipart POST /api/convert/ingest
              (file + division + team + year + seq=0 자동)
