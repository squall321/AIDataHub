...
### 9.3 kind 9종 — `api/schemas/attachment.py:29-51` 권위 출처
...
| `cad` | CAD 파일 | dwg/dxf/step/iges/stp/igs |
| `drawing` | 도면 / 벡터 드로잉 | vsd/vsdx/svg (드로잉 컨텍스트) |
| `data` | 정형 데이터 (JSON/XML/YAML 등) | json/xml/yaml/yml |
| `other` | 위에 속하지 않는 모든 첨부 (PPT 차트 placeholder 포함) | (자유) |

**확장자 ↔ kind 자동 매핑**: `api/schemas/attachment.py:54+` `_KIND_BY_EXT` 표 참조. 알 수 없는 확장자는 `other`.

**PPT 차트**: `ppt_converter/charts.py` 가 추출한 차트 데이터는 `tables[]` 1행으로 들어가고, 추가로 `attachments[].kind="other"` placeholder 가 등록된다 (`ppt_converter/core.py:684`). "chart" 라는 kind 값은 정의되어 있지 않다.
