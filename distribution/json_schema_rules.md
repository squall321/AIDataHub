...
| 필드 | 타입 | 필수 | 규칙 |
|------|------|------|------|
| `doc_id` | string | **필수** | 12장 형식 (`{prefix}-{division}-{team}-{year}-{seq6}`). 전사 고유. **변환기가 실제 출력하는 키**. normalizer 는 `meta.doc_id` 우선, 없으면 `meta.id` / `raw.id` 폴백. DB `records.id` 컬럼으로 매핑. |
| `id` | string | (alias) | 위 폴백 경로용. 사용 안 권장 — 신규 변환기는 `doc_id` 만 출력. |

> **참고**: `data_type` / `division` / `team` / `year` / `seq` 는 `doc_id` 에서 파싱으로 도출되며 **변환기는 별도 키로 출력하지 않는다**. RecordIn/RecordOut (Pydantic) 의 `data_type` / `division` / `team` / `year` / `seq` 필드는 normalizer 의 `parse_id()` (`api_server/src/api/schemas/id_format.py`) 가 채운다.

> **예외**: Excel DATA payload 만 top-level 에 `data_id` / `division` / `team` / `year` 를 별도 출력한다 (§11.2 참조).
...
| **Excel** | top-level | `data_id` / `schema_version="data.v1"` / `division` / `team` / `year` / `caption` / `headers` / `units` / `units_map` / `rows` / `row_count` / `column_count` / `source.{sheet,kind}` / `generated_at` / `warnings` | DATA payload 자체가 표준 변종. §11.2 참조 | `excel_converter/core.py:125-159` |
...
|---|---|---|---|---|---|---|
| `meta.doc_id` (DOC), `data_id` (top, Excel DATA) | `raw.id`, `meta.id` | string | required | `records.id` | VARCHAR(80) PK | 12장 형식. 파싱하여 division/team/year/seq 도출 (`schemas/id_format.parse_id`) |
...
| (id 파싱) | — | string | derived | `records.division` | VARCHAR(10) | RecordOut 응답에만 노출 |
...
- [ ] `meta.doc_id` (DOC 변종) 또는 top-level `data_id` (Excel DATA) 가 `{DATA_TYPE}-{DIV}-{TEAM}-{YYYY}-{6digits}` 형식인가
...
```json
{
  ...
  // 참고: data_type/division/team/year/seq 는 doc_id 파싱으로 도출되며 변환기는 출력하지 않음.
  // 응답 (RecordOut) 에서는 division="HE", team="CAE", year=2026, seq=1 로 노출됨.
